...
import {Edit, Eye, MoneyRecive, TickSquare, Trash} from "iconsax-react";
...
export default function TodayPlan({events}) {
    ...
    function priceCalculate(event) {
        const defaultPricePerHour = event.price ? event.price :
            event.customer.special_price_per_hour ? event.customer.special_price_per_hour :
    ...
    if (events.length !== 0) {
        total = events.map(event => priceCalculate(event)).reduce((total, num) => total + num);
    }
